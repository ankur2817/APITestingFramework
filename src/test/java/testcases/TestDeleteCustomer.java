...
public class TestDeleteCustomer extends BaseTest {
	...
	public void deleteCustomerAPI(String id) throws IOException {
		...
		Assert.assertTrue(TestUtil.jsonHasKey(response.asString(), "id"),
				"Test case failed as the id is not present in the response");

		Assert.assertEquals(TestUtil.getJSONKeyValue(response.asString(), "id"), id,
				"Test case failed as the id is not same");

		Assert.assertEquals(TestUtil.getJSONKeyValue(response.asString(), "deleted"), "true",
				"Test case failed as deleted key value is not true");
